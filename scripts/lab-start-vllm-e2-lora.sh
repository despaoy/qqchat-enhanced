#!/usr/bin/env bash
# E2 评估专用 vLLM 启动脚本：加载 Qwen3-8B-Instruct + E2 NEFTune LoRA
# 解决 Triton 编译 Python.h 缺失问题：借用 streamvggt_py311 的头文件
set -euo pipefail

ROOT=/home/szw/lhm2
# 用非 AWQ 的原版模型，与 LoRA 训练时的 base 一致
MODEL=$ROOT/runtime/models/Qwen3-8B-Instruct
ENV_DIR=$ROOT/envs/qqchat-gpu-qwen3

# E2 LoRA adapter
KISAKI_E2=$ROOT/runtime/loras/kisaki/e2_neftune_r32/final
...
if [[ ! -f "$KISAKI_E2/adapter_config.json" ]]; then
  echo "ERROR: E2 LoRA adapter not found at $KISAKI_E2" >&2
  exit 1
fi
...
exec env CUDA_VISIBLE_DEVICES=1 "$VLLM_BIN" serve "$MODEL" \
  --served-model-name kisaki-e2-neftune \
  --host 127.0.0.1 --port 8002 \
  --gpu-memory-utilization 0.90 --max-model-len 4096 \
  --enable-lora \
  --max-loras 1 \
  --max-lora-rank 32 \
  --lora-modules kisaki-e2-neftune="$KISAKI_E2" \
  --enforce-eager
