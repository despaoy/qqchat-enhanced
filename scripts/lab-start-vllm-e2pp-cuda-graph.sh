...
KISAKI_E2PP=$ROOT/runtime/loras/kisaki/e2pp_rag_r32/final
...
if [[ ! -f "$KISAKI_E2PP/adapter_config.json" ]]; then
  echo "ERROR: E2'' LoRA adapter not found at $KISAKI_E2PP" >&2
  exit 1
fi
...
exec env CUDA_VISIBLE_DEVICES=0 "$VLLM_BIN" serve "$MODEL" \
  ...
  --max-loras 1 \
  --max-lora-rank 32 \
  --lora-modules kisaki-e2pp-rag="$KISAKI_E2PP"
